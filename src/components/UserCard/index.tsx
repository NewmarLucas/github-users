    ...
    <div className='p-4 sm:py-10 sm:px-6 rounded-lg sm:rounded-md border flex flex-col gap-2 sm:gap-6 max-sm:divide-y'>
      <div className='flex sm:flex-col items-center gap-y-6 gap-x-2'>
        ...
        <div className='flex flex-col sm:items-center'>
          <h2 className='sm:text-h1 text-grey-neutral'>{user?.name ?? ''}</h2>
          <span className='text-md text-grey-dark'>@{user.login}</span>
        </div>
      </div>
      <p className='text-sm sm:text-md text-grey-dark sm:text-center max-sm:pt-2'>{user.bio}</p>
    </div>
  )
}
